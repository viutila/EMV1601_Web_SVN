...
ScheduleTable.prototype.setHx = function () {
    var args = arguments;
    //this.containerId = args[0];
    this.hx = args[0];
    //if (this.testCount % 2 == 0)
    //{
    //    this.hx.setNodeValue("NTSC", "Camera1", "VideoType");
    //} else {
    //    this.hx.setNodeValue("PAL", "Camera1", "VideoType");
    //}
    //this.testCount++;
...
ScheduleTable.prototype.setRowUI = function(tag_name,show_name){
	...
	for(var i = 1;i <= 6;i++){
		...
		if(tmpStart < 48){
			setIdInnerHTML("td_" + tag_name + "_" + tmpStart,i);//zone���s����1�}�l
		}
	}
};
				...
				ht.insertCell().className(this.tdEditClass);
					...
					if(this.hx.getNodeValue(this.camTag,tag_name,str,"recordType") == "0"){
                        ...
                        if (setHx.getNode("SCHEDULE", "NormalRate"))
                        {
                            ...
                            //check N or P 給適當的ARRAY  //add by GG
                                if (parent.g_model_name == "EPHD08+") {
                                    var isNP = this.hx.getNodeValue(this.camTag, "VideoType");
                                    //alert("isNP = " + isNP);
                                    if (isNP == "NTSC")//N
                                    {
                                        parent.setSelectFps(NorObj, parent.g_mainNormalSpeed_aryN);
                                    } else {
                                        parent.setSelectFps(NorObj, parent.g_mainNormalSpeed_aryP);
                                    }
                                } else {
                        ...
                        if (setHx.getNode("SCHEDULE", "EventRate"))
                        {
                            ...
                            if (parent.g_model_name == "EPHD08+") {
                                ...
                                if (isNP == "NTSC")//N
                                {
                                    parent.setSelectFps(EvenObj, parent.g_mainEventSpeed_aryN);
                                } else {
                                    parent.setSelectFps(EvenObj, parent.g_mainEventSpeed_aryP);
                                }
                            } else {
...
ScheduleTable.prototype.checkFps = function(){
    ...
    return true;//20121212  julia add �ثe��CHECK
