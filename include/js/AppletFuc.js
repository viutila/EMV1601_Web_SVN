...
function openLiveStreamApplet() {
    var layoutIdx;
    if (g_total_ch_num == 16 || g_total_ch_num == 32) {
        layoutIdx = 16;
    ...
    applet.setLiveInfo(appletWidth, appletHeight, layoutIdx, g_ip, g_port, g_total_ch_num); //setLiveInfo(int liveWidth , int liveHeight , int liveLayout , ip , port)
    applet.downloadMD5();
...
function openPlaybackStreamApplet() {   //******* for alarmwatch
    ...
    applet.appletRelease();
    applet.setUsernamePassword(g_user_name, g_user_password);
    applet.setLiveInfo(appletWidth, appletHeight - 50, layoutIdx, g_ip, g_port, g_total_ch_num); //setLiveInfo(int liveWidth , int liveHeight , int liveLayout , ip , port)
    applet.downloadMD5();
    applet.downloadDynamicLibrary();
    //applet.downloadEkb200Libraty();
    applet.setLiveUserRightCtrl(RemoteCtrlLiveChnmap);
...
function PBCancel01() {
    applet.pbCancel();
    applet.setLiveInfo(appletWidth, appletHeight, 0, g_ip, g_port, g_total_ch_num); //setLiveInfo(int liveWidth , int liveHeight , int liveLayout , ip , port)
    applet.initApplet();
    applet.openLiveStream(g_ip, g_port);
}
function changeViewMode(layoutMode) {
    if (layoutMode == 1) {
        ...
        var cValue;
        if (g_total_ch_num == 32){
        	cValue = 7;
        } else if (g_total_ch_num == 16) {
            cValue = 3;
        } else if (g_total_ch_num == 8) {
            cValue = 1;
        } else if (g_total_ch_num == 4) {
        ...
        applet.changeViewMode(livePage, layoutMode);
        ...
        applet.changeViewMode(livePage, layoutMode);
    } else if (layoutMode == 32) {
        if (layoutMode != liveMode) {
            liveMode = layoutMode;
            livePage = 0;
        }
        applet.changeViewMode(livePage, layoutMode);
    } else if (layoutMode == 36) {
        if (layoutMode != liveMode) {
            liveMode = layoutMode;
            livePage = 0;
        }
...
function initInstallCameraArray() {
    ...
    installCameraArray = new Array(g_total_ch_num);
    var hx_Get_Status = new HandleXml();
    hx_Get_Status.setUrl("./xml/Status.xml");
    var install_Status = hx_Get_Status.getNodeValue("install").toInt().toString(2);
    if (install_Status.length < g_total_ch_num) {
        install_Status = install_Status.toFill(g_total_ch_num);
    }
    for (var i = 0; i < g_total_ch_num; i++) {
        if (install_Status.substr(g_total_ch_num - 1 - i, 1) == "0") {
            installCameraArray[i] = false;
        } else if (install_Status.substr(g_total_ch_num - 1 - i, 1) == "1") {
            installCameraArray[i] = true;
        }
    }
    applet.setInstallValue(installCameraArray, g_total_ch_num);
}
